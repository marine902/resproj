"""
ABLATION A1 — Gaps bornés uniquement.
Améliorations actives : 1 (gaps [min-max] + coupure sur grand gap)
Améliorations désactivées :
    - Clustering Union-Find (toutes les séquences dans un seul cluster)
    - Filtres de qualité (clean_block appelé mais filter_yara_strings désactivé)
    - Paire médiane (alignement progressif k-LCS complet)
...
# ABLATION A1: cluster_samples désactivé — remplacé par un unique cluster contenant tous les échantillons
# (pas de regroupement Union-Find)
cluster_sample_bytes = 10000
cluster_threshold = 0.8  # non utilisé dans A1
...
# Paramètres filtres (présents mais non utilisés dans A1)
min_block_bytes = 8
...
# ABLATION A1: filter_yara_strings désactivé — retourne les strings sans second filtrage
def filter_yara_strings(strings: list[str], max_null_ratio: float = 0.3) -> list[str]:
    # ABLATION A1: filtrage désactivé, on retourne les strings telles quelles
    return strings


# Paramètres construction YARA
min_block_size = 16
max_gap_size = 50   # AMÉLIORATION 1 active : coupure si gap > 50
max_block_bytes = 500
max_strings_per_cluster = 20


def align_and_build_yara_strings(a: bytes, b: bytes, max_block_bytes: int = max_block_bytes) -> list[str]:
    """Build YARA strings from CIGAR alignment with bounded gaps (Amélioration 6)."""
    if len(a) > len(b):
        a, b = b, a
...
# ABLATION A1: yara_format_lcs utilisé pour formater le résultat du k-LCS progressif
# avec gaps bornés (Amélioration 1)
def yara_format_lcs(lcs: bytes, sequences: list[bytes], *, bytes_per_line: int = 24) -> list[str]:
    """
    Formats raw bytes into YARA hex strings with bounded [min-max] gaps (Amélioration 1).
    Used in A1 after progressive k-LCS alignment.
    """
    ...
    for i in range(len(lcs) - 1):
        ...
        if not contiguous_in_all:
            ...
            gap_max = max(gaps)
            if gap_max > 50:
                # AMÉLIORATION 1 : coupure sur grand gap (au lieu de [-])
                clean = clean_block(tokens)
                if clean is not None:
                    yara_strings.append(clean)
                tokens = [f"{lcs[i+1]:02x}"]
            else:
                # AMÉLIORATION 1 : gap borné [min-max]
...
def main():
    ap = argparse.ArgumentParser(description="ABLATION A1 — Bounded gaps only, progressive k-LCS, no clustering, no filters")
    ...
    logger = logging.getLogger("LCS_A1")
    ...
    try:
        ...
        # ABLATION A1: pas de clustering — un seul cluster = toutes les séquences
        clusters = [list(range(len(sequences)))]
        logger.info("A1: no clustering — single cluster with all sequences")
        ...
        for cluster in clusters:
            ...
            # ABLATION A1: alignement progressif k-LCS (pas de paire médiane, pas de mode local)
            if len(cluster_sequences) < 2:
                logger.warning("Cluster has only 1 sequence, skipping")
                continue
            lcs = k_lcs(cluster_sequences, logger=logger, workers=args.workers)
            yara_strings = yara_format_lcs(lcs, cluster_sequences)
            # ABLATION A1: filter_yara_strings désactivé (retourne tout tel quel)
        ...
        if not all_yara_strings:
            ...
            family_signatures_dirpath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "signatures_ablation", "A1", family)
