...
def cluster_samples(sequences: list[bytes],threshold_ratio: float = CLUSTER_THRESHOLD,pair_sample: int = PAIR_SAMPLE, logger: logging.Logger=None) -> list[list[int]]:
    """
...
def k_lcs_clustered(sequences: list[bytes],*,logger: logging.Logger,workers: int = 0,) -> list[str]:
    """
    ...
    clusters = cluster_samples(sequences, logger=logger)
...
def run_pipeline_on_family(family_name: str,zip_path: str,password: bytes,truncate: int,samples_goodware: list[bytes],logger: logging.Logger,) -> dict:
    ...
    try:
        sequences = collect_samples_from_zip(zip_path, family_name, password, truncate)
        if len(sequences) < 2:
        ...
        t0 = monotonic()
        yara_strings = k_lcs_clustered(sequences, logger=logger)
        result["nb_clusters"] = len(cluster_samples(sequences, logger=None))
        ...
        rule = yara_lib.compile(source=build_yara_rule_text(family_name, yara_strings, monotonic() - t0))
...
def main():
    ...
    ap.add_argument("--cluster-threshold",type=float,default=CLUSTER_THRESHOLD,help=f"Union-Find threshold as fraction of max distance (default: {CLUSTER_THRESHOLD})")
    ...
    logger.info(f"Cluster threshold: {args.cluster_threshold}")

    start_time = monotonic()
    sequences = collect_samples(family_dirpath, args.truncate_bytes, logger)
    yara_strings = k_lcs_clustered(sequences, logger=logger, workers=args.workers)
